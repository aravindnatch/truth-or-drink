...
const Game: NextPage = () => {
  ...
  useEffect(() => {
    ...
    for(let i = 0; i < unshuffled.length; i++) {
      let currentIndex = unshuffled[i].length,  randomIndex;
  
      while (currentIndex != 0) {
        randomIndex = Math.floor(Math.random() * currentIndex);
        currentIndex--;
    
        [unshuffled[i][currentIndex], unshuffled[i][randomIndex]] = [
          unshuffled[i][randomIndex], unshuffled[i][currentIndex]];
      }
    
      questionsarr.push(unshuffled[i])
    }

    setQuestions(questionsarr);
    setQuestion(questionsarr[bank][index]);
          ...
          <div 
            ...
            onClick={() => {
              if (index + 1 >= questions[bank].length) {
                ...
                setQuestion(questions[bank][index+1]); 
                setIndex(index + 1)
              }
            }}           
          >
